...
fn Composer(selected_model: RwSignal<String>) -> impl IntoView {
    ...
    view! { <div class="composer-dock"><form class="chat-composer" on:submit=move |event| { event.prevent_default(); submit(); } on:dragover=move |event: DragEvent| event.prevent_default() on:drop=move |event: DragEvent| { event.prevent_default(); if let Some(file) = event.data_transfer().and_then(|data| data.files()).and_then(|files| files.get(0)) { attach(runtime, chat, file); } }>
        <AttachmentChip />
        <div class="prompt-row"><label class="composer-button attach-button" aria-label="Add attachment"><Icon name="add" /><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden on:change=move |event| { if let Some(file) = event_target::<web_sys::HtmlInputElement>(&event).files().and_then(|files| files.get(0)) { attach(runtime, chat, file); } } /></label><textarea rows="2" placeholder="Message MiRMiR…" prop:value=move || prompt.get() on:input=move |event| prompt.set(event_target_value(&event)) on:keydown=move |event| { if event.key() == "Enter" && !event.shift_key() { event.prevent_default(); submit(); } } disabled=move || chat.running.get()></textarea><Show when=move || chat.running.get()><button class="composer-button cancel-button" type="button" aria-label="Stop generation" on:click=move |_| cancel_generation(runtime, chat)>"■"</button></Show><button class="composer-button send-button" type="submit" disabled=move || chat.running.get()>"↑"</button></div>
